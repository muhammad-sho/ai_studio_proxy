...
const OPENAI_ALIAS_PATH = /^\\/v1\\/(?:chat\\/completions|embeddings|images\\/generations|audio\\/(?:transcriptions|translations|speech)|files(?:\\/|$)|batches(?:\\/|$)|fine_tuning\\/jobs(?:\\/|$)|moderations)$/;
...
function isMetadataRoute(pathname, method = "GET", headers = {}) {
  ...
  if (/^\\/v1\\/(?:files|batches)(?:\\/|$)/.test(pathname)) return true;
  return false;
}
...
function classifyRoute(pathname, method, urlModel, action, body, headers) {
  if (isMetadataRoute(pathname, method, headers)) {
    return { kind: "metadata", model: null, trackUsage: false };
  }
