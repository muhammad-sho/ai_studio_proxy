function createAuth({ prep, crypto, trustProxy, sessionTtlMs, cookieSession, cookieCsrf, log, isOpenAiCompatibilityRoute }) {
  ...
  function resolveClientKey(request) {
    ...
    const bearer = isOpenAiCompatibilityRoute(url.pathname)
  ...
  function passwordResetCodeRecord() {
    const passwordHash = prep("SELECT value FROM app_meta WHERE key=?").get("password_reset_code_hash")?.value;
    const passwordSalt = prep("SELECT value FROM app_meta WHERE key=?").get("password_reset_code_salt")?.value;
    const expiresAt = Number(prep("SELECT value FROM app_meta WHERE key=?").get("password_reset_code_expires_at")?.value);
    return passwordHash && passwordSalt && Number.isSafeInteger(expiresAt) ? { passwordHash, passwordSalt, expiresAt } : null;
  }
