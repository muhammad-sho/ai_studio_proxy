...
function createDashboardAssets({ fs, zlib, json }) {
  const accessPageCache = new Map();
  const dashboardAssetCache = new Map();

  function accessPage(name) {
    if (!accessPageCache.has(name)) accessPageCache.set(name, fs.readFileSync(`dashboard/pages/${name}`, "utf8"));
    return accessPageCache.get(name);
  }
  ...
  function serveDashboardAsset(request, response, assetPath) {
    ...
    } else if (assetPath.startsWith("/panels/")) {
      const match = /^\/panels\/(overview|gemini-keys|client-keys|request-logs|statistics)\.(html|js)$/.exec(assetPath);
      if (!match) return json(response, 404, { error: "Not found" });
      file = `panels/${match[1]}.${match[2]}`;
      if (!fs.existsSync(`dashboard/${file}`)) return json(response, 404, { error: "Not found" });
    } else {
  ...
  return { accessPage, sendDashboard, serveDashboardAsset };
}
