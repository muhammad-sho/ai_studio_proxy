function createRequestHandler({
  crypto, db, prep, log, dbg, maskKey, json, securityHeaders, readBody,
  MAX_BODY_BYTES, SESSION_TTL_MS, REMEMBER_ME_TTL_MS,
  ...
  recordLog, hasAdmin, createSession, destroySession, destroyAllSessions,
  ...
  function requestIsSecure(request) {
    return request.headers["x-forwarded-proto"] === "https" || Boolean(request.socket && request.socket.encrypted);
  }

  function rememberCookieValue(rememberToken, secure) {
    return `${COOKIE_REMEMBER}=${rememberToken}; HttpOnly; Path=/; SameSite=Strict; Max-Age=${Math.floor(REMEMBER_ME_TTL_MS / 1000)}${secure}`;
  }

  // Persistent "remember me" sign-in: a valid remember cookie mints a fresh
  // short-lived dashboard session. The remember token itself is stable for its
  // 30-day lifetime (no per-request rotation) so parallel asset loads after a
  // session expiry cannot race each other. Tokens are 256-bit random values
  // stored hashed, HttpOnly + SameSite=Strict (+ Secure on HTTPS).
  function tryRememberLogin(request, response) {
    if (dashboardSessionValid(request)) return true;
    let presented = null;
    try { presented = cookieValue(request, COOKIE_REMEMBER); } catch { return false; }
    if (!presented) return false;
    const userId = rememberUserIdForToken(presented);
    if (!userId) {
      try {
        response.setHeader("Set-Cookie",
          `${COOKIE_REMEMBER}=; HttpOnly; Path=/; SameSite=Strict; Max-Age=0${requestIsSecure(request) ? "; Secure" : ""}`);
      } catch {}
      return false;
    }
    ...
    try {
      response.setHeader("Set-Cookie", [
        `${COOKIE_SESSION}=${token}; HttpOnly; Path=/; SameSite=Strict; Max-Age=${SESSION_TTL_MS / 1000}${secure}`,
        `${COOKIE_CSRF}=${csrfToken}; Path=/; SameSite=Strict; Max-Age=${SESSION_TTL_MS / 1000}${secure}`,
      ]);
    } catch {}
    const fresh = `${COOKIE_SESSION}=${token}; ${COOKIE_CSRF}=${csrfToken}`;
    try { request.headers.cookie = request.headers.cookie ? `${request.headers.cookie}; ${fresh}` : fresh; } catch {}
    log("info", "Auth", `remembered session restored for user #${userId} from ${clientAddress(request)} (session expires in ${SESSION_TTL_MS / 3600000}h)`);
    return true;
  }
  ...
  async function handleRequest(request, response) {
    ...
    if (apiRoute && apiRoute.subpath === "/models" && request.method === "GET") {
      if (!localKeyIsValid(request)) {
        const address = clientAddress(request);
        const traceId = crypto.randomUUID();
        log("warn", "Auth", `rejected ${request.method} ${url.pathname}: invalid client key from ${address}`);
        recordLog({
          model: "[metadata]",
          traceId,
          status: 401,
          ...
          events: [
            { t: 0, type: "receive", detail: `${request.method} ${url.pathname} from ${address}` },
            { t: 0, type: "reject", detail: "invalid client API key" },
          ],
        });
    ...
    if (url.pathname === "/" && request.method === "GET") {
      if (!hasAdmin()) { response.writeHead(200, { "Content-Type": "text/html; charset=utf-8" }); return response.end(accessPage("setup.html")); }
      if (!dashboardSessionValid(request) && !tryRememberLogin(request, response)) {
        response.writeHead(200, { "Content-Type": "text/html; charset=utf-8" });
        return response.end(accessPage("signin.html"));
      }
      return sendDashboard(request, response);
    }
    if (url.pathname === "/reset-password" && request.method === "GET") {
      if (!hasAdmin()) { response.writeHead(303, { Location: "/" }); return response.end(); }
      response.writeHead(200, { "Content-Type": "text/html; charset=utf-8" });
      return response.end(accessPage("reset-password.html"));
    }
    if (url.pathname === "/dashboard.css" || url.pathname === "/dashboard.js" || url.pathname.startsWith("/panels/")) {
      if (!dashboardSessionValid(request) && !tryRememberLogin(request, response)) return json(response, 401, { error: "Dashboard login required" });
    ...
    if (url.pathname === "/api/setup" && request.method === "POST") {
      ...
      if (String(body.passwordConfirmation || "") !== String(body.password || "")) return json(response, 400, { error: "Password confirmation does not match" });
    ...
    if (url.pathname === "/api/password-reset-code" && request.method === "POST") {
      const address = clientAddress(request);
      if (rateLimited(address)) return json(response, 429, { error: "Too many recovery attempts; try again later" });
      let body; try { body = JSON.parse((await readBody(request)).toString()); } catch { return json(response, 400, { error: "Invalid JSON" }); }
      const username = String(body.username || "");
      const user = /^[a-zA-Z0-9_.-]{3,64}$/.test(username)
        ? db.prepare("SELECT id FROM admin_users WHERE username = ?").get(username) : null;
      if (!user) {
        await passwordDigest("not-a-real-reset-code", "00000000000000000000000000000000");
        recordLoginFailure(address);
        return json(response, 202, { ok: true });
      }

      let resetCode = null;
      if (!passwordResetCodeActive()) {
        resetCode = await createPasswordResetCode();
        try {
          db.exec("BEGIN IMMEDIATE");
          if (!passwordResetCodeActive()) storePasswordResetCode(resetCode);
          else resetCode = null;
          db.exec("COMMIT");
        } catch (error) {
          try { db.exec("ROLLBACK"); } catch {}
      ...
      if (resetCode) log("warn", "Auth", `Password reset code for '${username}': ${resetCode.code}. It expires in 15 minutes.`);
      return json(response, 202, { ok: true });
    }
    if (url.pathname === "/api/reset-password" && request.method === "POST") {
      ...
      if (String(body.passwordConfirmation || "") !== password) return json(response, 400, { error: "Password confirmation does not match" });

      const user = /^[a-zA-Z0-9_.-]{3,64}$/.test(username)
        ? db.prepare("SELECT * FROM admin_users WHERE username = ?").get(username) : null;
      const resetCode = await passwordResetCodeValid(body.recoveryCode);
      if (!user) {
        await passwordValid(password, { password_salt: "00000000000000000000000000000000", password_hash: "00".repeat(64) });
      }
      if (!user || !resetCode) {
        recordLoginFailure(address);
        log("warn", "Auth", `failed local password recovery from ${address}`);
        return json(response, 401, { error: "Invalid username or recovery code" });
      }
      ...
      try {
        db.exec("BEGIN IMMEDIATE");
        if (!consumePasswordResetCode(resetCode)) {
          db.exec("ROLLBACK");
          recordLoginFailure(address);
          return json(response, 401, { error: "Invalid username or recovery code" });
        }
        db.prepare("UPDATE admin_users SET password_hash=?, password_salt=? WHERE id=?").run(passwordHash, passwordSalt, user.id);
        db.exec("COMMIT");
      } catch (error) {
        try { db.exec("ROLLBACK"); } catch {}
        throw error;
      ...
      destroyAllSessions();
      destroyRememberTokensForUser(user.id);
      clearLoginFailures(address);
      log("info", "Auth", `administrator password reset from ${address}; all dashboard sessions were signed out`);
    ...
    if (url.pathname === "/login" && request.method === "POST") {
      ...
      clearLoginFailures(address);
      const { token, csrfToken } = createSession();
      const rememberRequested = rememberMeRequested(body.rememberMe ?? body.remember_me ?? body.remember);
      const secure = requestIsSecure(request) ? "; Secure" : "";
      const cookies = [
        `${COOKIE_SESSION}=${token}; HttpOnly; Path=/; SameSite=Strict; Max-Age=${SESSION_TTL_MS / 1000}${secure}`,
        `${COOKIE_CSRF}=${csrfToken}; Path=/; SameSite=Strict; Max-Age=${SESSION_TTL_MS / 1000}${secure}`,
      ...
      if (rememberRequested) {
        const { token: rememberToken } = createRememberToken(user.id);
        cookies.push(rememberCookieValue(rememberToken, secure));
        log("info", "Auth", `user '${username}' logged in from ${address} with remember-me (session expires in ${SESSION_TTL_MS / 3600000}h, remembered for ${Math.round(REMEMBER_ME_TTL_MS / 86400000)}d)`);
      } else {
        const stale = (() => { try { return cookieValue(request, COOKIE_REMEMBER); } catch { return null; } })();
        if (stale) {
          destroyRememberToken(stale);
          cookies.push(`${COOKIE_REMEMBER}=; HttpOnly; Path=/; SameSite=Strict; Max-Age=0${secure}`);
        }
        log("info", "Auth", `user '${username}' logged in from ${address} (session expires in ${SESSION_TTL_MS / 3600000}h)`);
      }
      response.writeHead(302, { Location: "/", "Cache-Control": "no-store", "Set-Cookie": cookies }); return response.end();
    }
    if (url.pathname === "/logout" && request.method === "POST") {
      if (!csrfValid(request)) return json(response, 403, { error: "Invalid CSRF token" });
      const presented = (() => { try { return cookieValue(request, COOKIE_REMEMBER); } catch { return null; } })();
      if (presented) destroyRememberToken(presented);
      destroySession(request);
      log("info", "Auth", `user logged out`);
      const logoutSecure = requestIsSecure(request) ? "; Secure" : "";
      response.writeHead(303, { Location: "/", "Cache-Control": "no-store", "Set-Cookie": [`${COOKIE_SESSION}=; HttpOnly; Path=/; SameSite=Strict; Max-Age=0${logoutSecure}`, `${COOKIE_CSRF}=; HttpOnly; Path=/; SameSite=Strict; Max-Age=0${logoutSecure}`, `${COOKIE_REMEMBER}=; HttpOnly; Path=/; SameSite=Strict; Max-Age=0${logoutSecure}`] });
      return response.end();
    }
    if (url.pathname.startsWith("/api/admin") && !dashboardSessionValid(request) && !tryRememberLogin(request, response)) {
    ...
    if (url.pathname === "/api/admin/state" && request.method === "GET") {
      ...
      const balanceScore = routingBalanceScore(usage, keys);
      ...
      return json(response, 200, { keys, clientKeys, usage, balanceScore, resetAt: new Date(pacificDayStart()).toISOString(), resetTimezone: "America/Los_Angeles", modelsCheckedAt: null, models: [], cooldowns });
    }
    ...
    if (url.pathname === "/api/admin/logs" && request.method === "DELETE") {
      let body;
      try { body = JSON.parse((await readBody(request)).toString()); } catch { return json(response, 400, { error: "Confirmation required" }); }
      if (body.confirm !== "DELETE LOGS") return json(response, 400, { error: "Confirmation required" });
      const deleted = db.prepare("DELETE FROM request_logs").run().changes;
      log("warn", "Admin", `deleted ${deleted} request log entr${deleted === 1 ? "y" : "ies"}`);
    ...
    if (url.pathname === "/api/admin/usage" && request.method === "DELETE") {
      let body;
      try { body = JSON.parse((await readBody(request)).toString()); } catch { return json(response, 400, { error: "Confirmation required" }); }
      if (body.confirm !== "DELETE USAGE") return json(response, 400, { error: "Confirmation required" });
      const deleted = db.prepare("DELETE FROM usage").run().changes;
      log("warn", "Admin", `deleted ${deleted} usage entr${deleted === 1 ? "y" : "ies"}`);
      return json(response, 200, { ok: true, deleted });
    }
    if (url.pathname === "/api/admin/usage" && request.method === "GET") {
      ...
      const fromStart = pacificDateStart(fromParam);
      const toParam = (url.searchParams.get("to") || "").trim();
      const toEnd = pacificDateEnd(toParam);
      if (!allowedPeriods.has(period)) period = "30d";
      let start = 0;
      let end = Number.MAX_SAFE_INTEGER;
      let scope;
      if (fromStart || toEnd) {
        if (fromStart) start = fromStart;
        if (toEnd) end = toEnd;
        scope = fromStart && toEnd
          ? `${fromParam} to ${toParam} (Pacific)`
          : fromStart ? `since ${fromParam} (Pacific)` : `through ${toParam} (Pacific)`;
      } else if (/^\d{4}-(0[1-9]|1[0-2])$/.test(monthParam)) {
