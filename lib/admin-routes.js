function createRequestHandler({
  ...
  function tryRememberLogin(request, response) {
    ...
    try {
      response.setHeader("Set-Cookie", [
        `${COOKIE_SESSION}=${token}; HttpOnly; SameSite=Strict; Max-Age=${SESSION_TTL_MS / 1000}${secure}`,
        `${COOKIE_CSRF}=${csrfToken}; SameSite=Strict; Max-Age=${SESSION_TTL_MS / 1000}${secure}`,
      ]);
    } catch {}
  ...
  async function handleRequest(request, response) {
    ...
    if (url.pathname === "/logout" && request.method === "POST") {
      ...
      response.writeHead(303, { Location: "/", "Cache-Control": "no-store", "Set-Cookie": [`${COOKIE_SESSION}=; HttpOnly; SameSite=Strict; Max-Age=0${logoutSecure}`, `${COOKIE_CSRF}=; HttpOnly; SameSite=Strict; Max-Age=0${logoutSecure}`, `${COOKIE_REMEMBER}=; HttpOnly; Path=/; SameSite=Strict; Max-Age=0${logoutSecure}`] });
      return response.end();
    }
