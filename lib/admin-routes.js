function createRequestHandler({
  ...
  const MODEL_USAGE_FILTER = `{prefix}model NOT LIKE '/%' AND {prefix}model NOT LIKE '[metadata] %'
    AND {prefix}model NOT LIKE '[untracked endpoint]%' AND {prefix}model NOT LIKE '[unidentified inference]%'`;
  const usageModelFilter = (prefix = "") => MODEL_USAGE_FILTER.replaceAll("{prefix}", prefix ? `${prefix}.` : "");
  function displayLogModel(value) {
    const model = String(value || "").trim();
    if (!model) return "[untracked endpoint]";
    if (model.startsWith("[metadata]")) return "[metadata]";
    if (model.startsWith("[untracked endpoint]")) return "[untracked endpoint]";
    if (model.startsWith("[unidentified inference]")) return "[unidentified inference]";
    if (model.startsWith("/")) return "[untracked endpoint]";
    return model;
  }
  function logModelFilter(model) {
    if (model === "[metadata]") return "(model = '[metadata]' OR model LIKE '[metadata] %')";
    if (model === "[untracked endpoint]") return "(model LIKE '/%' OR model LIKE '[untracked endpoint]%')";
    if (model === "[unidentified inference]") return "(model LIKE '[unidentified inference]%')";
    return "model = ?";
  }
  ...
  async function handleRequest(request, response) {
    ...
    if (url.pathname === "/reset-password" && request.method === "GET") {
      ...
      response.writeHead(200, { "Content-Type": "text/html; charset=utf-8" });
      return response.end(staticPage("reset-password.html"));
    }
    ...
    if (url.pathname === "/api/admin/state" && request.method === "GET") {
      ...
      return json(response, 200, { keys, clientKeys, usage, balanceScore, resetAt: new Date(pacificDayStart()).toISOString(), resetTimezone: "America/Los_Angeles", modelsCheckedAt: getMeta("models_checked_at"), models, cooldowns });
    }
    ...
    if (url.pathname === "/api/admin/logs" && request.method === "GET") {
      ...
      const where = [];
      const params = [];
      if (model) { where.push(logModelFilter(model)); if (!model.startsWith("[")) params.push(model); }
      ...
      const rawLogs = db.prepare(`SELECT id, created_at, model, key_label, key_masked, status, outcome, error_code, attempt, trace_id FROM request_logs${whereSql} ORDER BY id DESC LIMIT ? OFFSET ?`).all(...params, limit, offset);
      const logs = rawLogs.map((entry) => ({ ...entry, model: displayLogModel(entry.model) }));
      const total = db.prepare(`SELECT COUNT(*) AS c FROM request_logs${whereSql}`).get(...params).c;
      const logModels = [...new Set(db.prepare("SELECT model FROM request_logs").all().map((row) => displayLogModel(row.model)))].sort();
    ...
    if (logMatch && request.method === "GET") {
      ...
      entry.model = displayLogModel(entry.model);
    ...
    if (url.pathname === "/api/admin/usage" && request.method === "GET") {
      ...
      if (view === "all" || view === "clients") {
        payload.clients = prep(`SELECT u.client_key_id AS id, COALESCE(k.label, '(deleted #' || u.client_key_id || ')') AS label,
          ...
          WHERE u.created_at >= ? AND u.created_at < ? AND ${usageModelFilter("u")} GROUP BY u.client_key_id ORDER BY total DESC`).all(start, end);
        payload.matrix_client = prep(`SELECT COALESCE(k.label, '(deleted #' || u.client_key_id || ')') AS label,
            u.model AS model, COUNT(*) AS total
          FROM usage u LEFT JOIN client_keys k ON k.id = u.client_key_id
          WHERE u.created_at >= ? AND u.created_at < ? AND ${usageModelFilter("u")} GROUP BY u.client_key_id, u.model ORDER BY total DESC`).all(start, end);
      }
      if (view === "all" || view === "gemini") {
        payload.keys = prep(`SELECT u.gemini_key_id AS id, COALESCE(k.label, '(deleted #' || u.gemini_key_id || ')') AS label,
          ...
          WHERE u.created_at >= ? AND u.created_at < ? AND ${usageModelFilter("u")} GROUP BY u.gemini_key_id ORDER BY total DESC`).all(start, end);
        payload.matrix_gemini = prep(`SELECT COALESCE(k.label, '(deleted #' || u.gemini_key_id || ')') AS label,
            u.model AS model, COUNT(*) AS total
          FROM usage u LEFT JOIN api_keys k ON k.id = u.gemini_key_id
          WHERE u.created_at >= ? AND u.created_at < ? AND ${usageModelFilter("u")} GROUP BY u.gemini_key_id, u.model ORDER BY total DESC`).all(start, end);
      }
      if (view === "all" || view === "statistics") {
        payload.models = prep(`SELECT model, COUNT(*) AS total, SUM(ok) AS success, COUNT(*) - SUM(ok) AS failed
          FROM usage WHERE created_at >= ? AND created_at < ? AND ${usageModelFilter()} GROUP BY model ORDER BY total DESC`).all(start, end);
        payload.failures_model = prep(`SELECT model, IFNULL(error_code, 'unknown') AS code, COUNT(*) AS n
          FROM usage WHERE ok = 0 AND created_at >= ? AND created_at < ? AND ${usageModelFilter()} GROUP BY model, error_code ORDER BY n DESC`).all(start, end);
      }
