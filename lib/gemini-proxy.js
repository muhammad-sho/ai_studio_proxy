function createGeminiProxy({
  ...
  const classify = classifyRoute || ((pathname, method, urlModel, action, body) => ({ kind: "inference", model: urlModel || null, trackUsage: true, statsModel: urlModel ? (action ? statsModelName(urlModel, action, pathname) : urlModel) : pathname }));
  ...
  // Model discovery is intentionally a live, non-metered pass-through. Model lists are
  // small and requested rarely, so serving Google's current response is more useful
  // than maintaining a potentially stale response cache. A single configured key is
  // used because discovery does not consume inference quota or affect load balancing.
  async function refreshModelsOnce(context) {
    ...
    try {
      const result = await forwardToGemini(context, Buffer.alloc(0), key.api_key);
      syncModelsForDashboard(result);
      return result;
    } catch (error) {
  ...
  // Keep the dashboard's model-name catalog current after a complete live list
  // succeeds. This metadata is never read by the client discovery path.
  function syncModelsForDashboard(result) {
    if (!result || result.status < 200 || result.status >= 300 || !result.body) return;
    let payload;
    try { payload = JSON.parse(result.body.toString("utf8")); } catch { return; }
    if (!Array.isArray(payload.models) || payload.nextPageToken) return;
    const names = [...new Set(payload.models
      .map((model) => String(model?.name || "").replace(/^models\//, "").trim())
      .filter(Boolean))];
    if (!names.length) return;
    prep("DELETE FROM models").run();
    const insert = prep("INSERT INTO models (name) VALUES (?) ON CONFLICT(name) DO NOTHING");
  ...
  async function handleModelsList(request, response) {
    const result = await refreshModelsOnce(contextFromRequest(request));
  ...
  async function handleGeminiPassthrough(request, response, model, action) {
    ...
    const routePolicy = classify(pathname, request.method, model, action, body, request.headers);
