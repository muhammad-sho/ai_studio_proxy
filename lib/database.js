function createDatabase({ DatabaseSync, dbPath, fs }) {
  ...
  db.exec(`
    ...
    CREATE TABLE IF NOT EXISTS dashboard_remember_tokens (
      token_hash TEXT PRIMARY KEY,
      user_id INTEGER NOT NULL,
      created_at INTEGER NOT NULL,
      expires_at INTEGER NOT NULL
    );
    CREATE INDEX IF NOT EXISTS idx_remember_tokens_user ON dashboard_remember_tokens(user_id);
    CREATE INDEX IF NOT EXISTS idx_remember_tokens_expires ON dashboard_remember_tokens(expires_at);
  `);

  const usageColumns = db.prepare("PRAGMA table_info(usage)").all();
  if (!usageColumns.some((column) => column.name === "latency_ms")) {
    db.exec("ALTER TABLE usage ADD COLUMN latency_ms INTEGER");
  }
